...
contract StratManager is Ownable, Pausable {
    ...
    address public unirouter = address(0x66153fDc998252C0A98764933e2fC8D1B1009C2B);
...
contract StrategyLPToken is StratManager, FeeManager, GasThrottler {
    ...
    address constant public xtoken = address(0x2Eddba8b949048861d2272068A94792275A51658);  // BECO, token earn (ex. Beco or DFL or ...)
    address constant public want = address(0xd91E71E3486900Eaa41fa37BdD2a161bcDf0AB7e); // KPHI-KAI
    address public lpToken0;
    address public lpToken1;

    // Third party contracts
    address constant public masterchef = address(0x20e8Ff1e1d9BC429489dA76B1Fc20A9BFbF3ee7e); // Beco masterchef
    uint256 public poolId = 11;
    // Referrer
    address public referrer = address(0x0);

    // Routes
    address[] public xtokenTowkaiRoute = [xtoken, wkai];
    address[] public xtokenToLp0Route;
    address[] public xtokenToLp1Route;
    ...
    constructor(
        ...
        address _strategist,
        address _sleepEarnFeeRecipient
    ) StratManager(_keeper, _strategist, _sleepEarnFeeRecipient) public {
        lpToken0 = IKAIDexPair(want).token0();
        lpToken1 = IKAIDexPair(want).token1();

        if (lpToken0 == wkai) {
            xtokenToLp0Route = [xtoken, wkai];
        } else if (lpToken0 != xtoken) {
            xtokenToLp0Route = [xtoken, wkai, lpToken0];
        }

        if (lpToken1 == wkai) {
            xtokenToLp1Route = [xtoken, wkai];
        } else if (lpToken1 != xtoken) {
            xtokenToLp1Route = [xtoken, wkai, lpToken1];
        }
    ...
    function chargeFees() internal {
        uint256 towkai = IERC20(xtoken).balanceOf(address(this)).mul(45).div(1000);
        IKAIDexRouter(unirouter).swapExactTokensForTokens(towkai, 0, xtokenTowkaiRoute, address(this), now);
    ...
    function addLiquidity() internal {
        uint256 becoHalf = IERC20(xtoken).balanceOf(address(this)).div(2);

        if (lpToken0 != xtoken) {
            IKAIDexRouter(unirouter).swapExactTokensForTokens(becoHalf, 0, xtokenToLp0Route, address(this), now);
        }

        if (lpToken1 != xtoken) {
            IKAIDexRouter(unirouter).swapExactTokensForTokens(becoHalf, 0, xtokenToLp1Route, address(this), now);
        }
