...
contract StrategyDflLP is StratManager, FeeManager, GasThrottler {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address constant public wkai = address(0xAF984E23EAA3E7967F3C5E007fbe397D8566D23d);
    address constant public dfl = address(0xD675fF2B0ff139E14F86D87b7a6049ca7C66d76e);
    address public want;
    address public lpToken0;
    address public lpToken1;

    // Third party contracts
    address constant public masterchef = address(0x0245a1f57Ee84b55Cf489Eb5F3d27355014e57f8);
    uint256 public poolId;

    // Routes
    address[] public becoTowkaiRoute = [dfl, wkai];
    ...
    constructor(
        ...
        if (lpToken0 == wkai) {
            becoToLp0Route = [dfl, wkai];
        } else if (lpToken0 != dfl) {
            becoToLp0Route = [dfl, wkai, lpToken0];
        }

        if (lpToken1 == wkai) {
            becoToLp1Route = [dfl, wkai];
        } else if (lpToken1 != dfl) {
            becoToLp1Route = [dfl, wkai, lpToken1];
        }
    ...
    function chargeFees() internal {
        uint256 towkai = IERC20(dfl).balanceOf(address(this)).mul(45).div(1000);
    ...
    function addLiquidity() internal {
        uint256 becoHalf = IERC20(dfl).balanceOf(address(this)).div(2);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(dfl).safeApprove(unirouter, uint256(-1));
    ...
    function _removeAllowances() internal {
        ...
        IERC20(dfl).safeApprove(unirouter, 0);
        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, 0);
    }
}